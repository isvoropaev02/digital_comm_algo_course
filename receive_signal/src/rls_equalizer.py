...
class Equalizer:
    def __init__(self, mu: float = 0.9, filter_order: int = 15) -> None:
        self.__mu = mu
        self.__eq_w = np.zeros(filter_order, dtype=np.complex64)
        self.__p_mat = np.eye(filter_order, dtype=np.complex64)
    ...
    def __train_filter(self, in_samples: np.ndarray, ref_samples: np.ndarray) -> None:
        ...
        for j_smp in range(len(in_samples)):
            ...
            Px = np.dot(self.__p_mat, self.__x_buff)
            k = Px / (self.__mu + np.inner(np.conj(self.__x_buff), Px))
            # print(np.abs(err))
            self.__eq_w = self.__eq_w + k * np.conj(err)
            self.__p_mat = (self.__p_mat + np.dot(np.outer(k, np.conj(self.__x_buff)), self.__p_mat)) / self.__mu
