...
# configuration
cfg = ConfigParams(fs_hz=16000, fc_hz=0, fsymb_hz=1800)
...
corr = np.correlate(resampled_bb_signal, preambule_modulated, "full")
# plot_signal(np.abs(corr), x_scale_coef=1 / fs_hz_new)
...
# plot_signal(np.abs(corr_m), x_scale_coef=1 / fs_hz_new)

plt.figure()
plt.suptitle("Correlation")
plt.plot(np.abs(corr), label="A")
